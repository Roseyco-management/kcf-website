...
export default async function AnalyticsPage() {
  ...
  return (
    <div className="space-y-6">
      ...
      {/* Integration Status */}
      <div className="rounded-2xl bg-white p-6 shadow-sm">
        <h2 className="mb-6 text-xl font-semibold text-gray-900">Integration Status</h2>
        <div className="grid grid-cols-1 gap-4 md:grid-cols-2 lg:grid-cols-4">
          {/* Meta CAPI */}
          <div className="rounded-xl border-2 border-gray-200 p-4">
            <div className="flex items-center justify-between mb-3">
              <h3 className="font-semibold text-gray-900">Meta CAPI</h3>
              {meta.configured ? (
            ...
            <p className="text-xs text-gray-600 mb-3">Pixel ID: {meta.pixelId}</p>
            {meta.configured && (
              <div className="space-y-1 text-sm">
                <div className="flex justify-between">
                  <span className="text-gray-600">Page Views:</span>
                  <span className="font-medium">{safeNumber(meta.events.pageView).toLocaleString()}</span>
                </div>
                <div className="flex justify-between">
                  <span className="text-gray-600">Conversions:</span>
                  <span className="font-medium">{safeNumber(meta.conversions)}</span>
                </div>
                <div className="flex justify-between">
                  <span className="text-gray-600">CVR:</span>
                  <span className="font-medium">{safeNumber(meta.conversionRate).toFixed(1)}%</span>
                </div>
              </div>
            )}
          </div>
          ...
          <div className="rounded-xl border-2 border-gray-200 p-4">
            ...
            <p className="text-xs text-gray-600 mb-3">ID: {ga4.measurementId}</p>
            {ga4.configured && (
              <div className="space-y-1 text-sm">
                <div className="flex justify-between">
                  <span className="text-gray-600">Sessions:</span>
                  <span className="font-medium">{safeNumber(ga4.sessions).toLocaleString()}</span>
                </div>
                <div className="flex justify-between">
                  <span className="text-gray-600">Users:</span>
                  <span className="font-medium">{safeNumber(ga4.users).toLocaleString()}</span>
                </div>
                <div className="flex justify-between">
                  <span className="text-gray-600">Bounce Rate:</span>
                  <span className="font-medium">{safeNumber(ga4.bounceRate).toFixed(1)}%</span>
                </div>
              </div>
            )}
          </div>
          ...
          <div className="rounded-xl border-2 border-gray-200 p-4">
            ...
            <p className="text-xs text-gray-600 mb-3">Project: {clarity.projectId}</p>
            {clarity.configured && (
              <div className="space-y-1 text-sm">
                <div className="flex justify-between">
                  <span className="text-gray-600">Sessions:</span>
                  <span className="font-medium">{safeNumber(clarity.sessions).toLocaleString()}</span>
                </div>
                <div className="flex justify-between">
                  <span className="text-gray-600">Recordings:</span>
                  <span className="font-medium">{safeNumber(clarity.recordings).toLocaleString()}</span>
                </div>
                <div className="flex justify-between">
                  <span className="text-gray-600">Heatmaps:</span>
                  <span className="font-medium">{safeNumber(clarity.heatmaps)}</span>
                </div>
              </div>
            )}
          </div>
          ...
          <div className="rounded-xl border-2 border-primary/20 bg-primary/5 p-4">
            <div className="flex items-center justify-between mb-3">
              <h3 className="font-semibold text-gray-900">RoseyCo SEO</h3>
              {roseyco.configured ? (
                <CheckCircle className="h-5 w-5 text-green-500" />
              ) : (
            ...
            <p className="text-xs text-gray-600 mb-3">Client: KCF</p>
            {!roseyco.configured && (
              <p className="text-xs text-gray-600">{roseyco.message}</p>
            )}
          </div>
        </div>
      </div>
